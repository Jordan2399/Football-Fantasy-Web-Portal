...
import { ForgotPasswordSchema, ForgotPasswordSchemaType } from "@/validation/form.validation";
...
export const ForgotPasswordScreenForm = () => {
  ...
  const { mutateAsync, isPending } = useMutation<
    AuthenticationServicesType.ForgotPasswordRes,
    Error,
    AuthenticationServicesType.ForgotPasswordProps
  >({
    mutationFn: (variables) => AuthenticationServices.ForgotPassword(variables),
    onSuccess: (data) => {
  ...
  return (
    ...
    >
      {confirmScreen && (
        <div
          className={
            "mb-12 w-full h-fit flex flex-col items-center justify-center gap-2"
          }
        >
          <svg
